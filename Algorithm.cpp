...
std::vector<Lexem *> parse_lexem(std::string &string) {
	...
	for (int i = 0; i < string.size(); ++i) {
		if (string[i] == ' ') {
		...
		else if (string[i] == 'g' && string[i + 1] == 'o' && string[i + 2] == 't' && string[i + 3] == 'o') {
			i += 4;
			while (string[i] == ' ') {
				++i;
			}
			std::string obj = "";
			while (string[i] == '_' || (string[i] >= 'a' && string[i] <= 'z') || (string[i] >= 'A' && string[i] <= 'Z') || (string[i] >= '0' && string[i] <= '9')) {
				obj += string[i];
				++i;
			}
			--i;
			list_lex.push_back(new Goto(obj));

		}
		else if (string[i] == 'i' && string[i + 1] == 'f') {
			i += 2;
			list_lex.push_back(new If);
		}
		else if (string[i] == 'e' && string[i + 1] == 'l' && string[i + 2] == 's' && string[i + 3] == 'e') {
			i += 4;
			list_lex.push_back(new Else);
		}
		else if (string[i] == 'w' && string[i + 1] == 'h' && string[i + 2] == 'i' && string[i + 3] == 'l' && string[i + 4] == 'e') {
			i += 5;
			static int tmp = 0;
			Label *ptr = new Label("$while" + std::to_string(tmp));
			list_lex.push_back(ptr);
			list_lex.push_back(new While(tmp));
			++tmp;
		}
		else if (string[i] == 'p' && string[i + 1] == 'r' && string[i + 2] == 'i' && string[i + 3] == 'n' && string[i + 4] == 't') {
			i += 5;
			list_lex.push_back(new Print);
		}
		else if (string[i] == 'n' && string[i + 1] == 'e' && string[i + 2] == 'w' && string[i + 3] == 'l' && string[i + 4] == 'i' && string[i + 5] == 'n' && string[i + 6] == 'e') {
			i += 7;
			list_lex.push_back(new New_Line);
		}
		else if (string[i] == 'r' && string[i + 1] == 'e' && string[i + 2] == 't' && string[i + 3] == 'u' && string[i + 4] == 'r' && string[i + 5] == 'n') {
			i += 6;
			list_lex.push_back(new Return);
		}
		else if (string[i] == 'd' && string[i + 1] == 'e' && string[i + 2] == 'f') {
        		i += 3;
			...
			while (string[i] != ')') {
				...
				arguments.push_back(arg_name);
	            	...
	            	list_lex.push_back(new Function(obj, arguments, true));
        	}
		else if (string[i] == 'e' && string[i + 1] == 'n' && string[i + 2] == 'd' && string[i + 3] == 'e' && string[i + 4] == 'f') {
			i += 5;
			list_lex.push_back(new EndFunction());
		}
		else if (string[i] == ',') {
			list_lex.push_back(new Comma);
		}
			...
			if (string[i] == ':') {//исправить
				list_lex.push_back(new Label(obj));
				continue;
			}
			if (string[i] != '_' && !(string[i] >= 'a' && string[i] <= 'z') && !(string[i] >= 'A' && string[i] <= 'Z')) {
				list_lex.push_back(new Number(stoi(obj)));
				--i;
				continue;
			}
			while (string[i] == '_' || (string[i] >= 'a' && string[i] <= 'z') || (string[i] >= 'A' && string[i] <= 'Z')) {
				obj += string[i];
				++i;
			}
			if (string[i] == ':') {
			...
			if (string[i] == '[') {
				list_lex.push_back(new Array(obj));
				list_lex.push_back(new RLBracket());
				continue;

			}
			if (string[i] == '(') {
				list_lex.push_back(new Function(obj, false));
				list_lex.push_back(new LBracket());
				continue;
			}
			--i;
			list_lex.push_back(new Variable(obj));
		}
		else if (string[i] == '+') {
			list_lex.push_back(new Plus);
		}
		...
		else if (string[i] == '_' || (string[i] >= 'a' && string[i] <= 'z') || (string[i] >= 'A' && string[i] <= 'Z')) {
			std::string obj = "";
			while (string[i] == '_' || (string[i] >= 'a' && string[i] <= 'z') || (string[i] >= 'A' && string[i] <= 'Z') || (string[i] >= '0' && string[i] <= '9')) {
				obj += string[i];
				++i;
...
std::vector<Lexem *> build_poliz(std::vector<Lexem *> &infix) {
	...
	for (int i = 0; i < infix.size(); ++i) {
		if (infix[i] -> get_type() == NUMBER) {
			...
			if (infix[i] -> get_type() == FLBRACKET || infix[i] -> get_type() == FRBRACKET) {
					while (!stack.empty()) {
						poliz.push_back(stack.top());
						stack.pop();
					}
